 ...
 */
export async function setupMockApi(page: Page, options: { isAuthenticated?: boolean } = { isAuthenticated: true }) {
  let isAuthed = options.isAuthenticated ?? true;
  const portfolios = [...SEED_PORTFOLIOS];
  const alertRules = [...SEED_ALERTS.rules];
  const reports = [...SEED_REPORTS];
  ...
  await page.route('**/api/v1/**', async (route) => {
        ...
        body: JSON.stringify(
          successEnvelope(
            alertRules.map((r: any) => ({
              ...r,
              alertType: r.alertType || (r.ruleType === 'DRAWDOWN' ? 'DRAWDOWN_LIMIT' : 'RISK_SCORE_SPIKE'),
    ...
    if (path.endsWith('/reports/csv') && method === 'POST') {
      return route.fulfill({
        ...
        headers: {
          'Content-Disposition': 'attachment; filename="wealthcompass-portfolio-summary-p-seed-001.csv"',
