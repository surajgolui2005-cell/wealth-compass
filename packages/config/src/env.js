...
exports.envSchema = zod_1.z
  .object({
    ...
    WEB_PORT: zod_1.z.coerce.number().int().positive().default(5173),
    QUANT_ENGINE_PORT: zod_1.z.coerce.number().int().positive().default(8000),
    ADMINER_PORT: zod_1.z.coerce.number().int().positive().default(8080),
    // PostgreSQL Database
    POSTGRES_HOST: zod_1.z.string().min(1, "POSTGRES_HOST is required").default("localhost"),
    POSTGRES_PORT: zod_1.z.coerce.number().int().positive().default(5432),
    POSTGRES_USER: zod_1.z.string().min(1, "POSTGRES_USER is required").default("postgres"),
    POSTGRES_PASSWORD: zod_1.z.string().min(1, "POSTGRES_PASSWORD is required"),
    POSTGRES_DB: zod_1.z.string().min(1, "POSTGRES_DB is required").default("investor_pm"),
    DATABASE_URL: zod_1.z.string().min(1).optional(),
    // Redis Cache & Queue
    REDIS_HOST: zod_1.z.string().min(1, "REDIS_HOST is required").default("localhost"),
    ...
    QUANT_ENGINE_URL: zod_1.z.string().url().default("http://localhost:8000"),
    ...
    COINGECKO_API_KEY: zod_1.z.string().optional().default(""),
    OPEN_EXCHANGE_RATES_APP_ID: zod_1.z.string().optional().default(""),
    ZERODHA_API_KEY: zod_1.z.string().optional().default(""),
    ZERODHA_API_SECRET: zod_1.z.string().optional().default(""),
    BINANCE_API_KEY: zod_1.z.string().optional().default(""),
    BINANCE_API_SECRET: zod_1.z.string().optional().default(""),
  })
  .transform((data) => {
